...
let
  cfg = config.hardware.strix-halo.ryzenadj;
...
in
{
  options.hardware.strix-halo.ryzenadj = {
    enable = lib.mkEnableOption "Ryzen power management tuning via ryzenadj";
    ...
    curveOptimizer = {
      enable = lib.mkEnableOption "curve optimizer undervolting (negative = more efficient)";

      offset = lib.mkOption {
        type = lib.types.int;
        default = 0;
        example = -30;
        description = "All-core Curve Optimizer offset. Negative = undervolt, positive = overvolt.";
      };
      ...
      graceSeconds = lib.mkOption {
        ...
        default = 60;
        example = 120;
        description = "Seconds to wait before applying curve optimizer (allows recovery if unstable). Set to 0 to disable.";
      };
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.ryzenadj = lib.mkIf hasAnyConfig {
      description = "Configure AMD Ryzen power limits";
