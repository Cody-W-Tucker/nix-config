# EC-SU_AXB35 embedded controller for GMKtec EVO-X2
# Provides fan control, power modes, and thermal monitoring via sysfs
{
  config,
  lib,
  pkgs,
  ...
  fanOptions = {
    options = {
      mode = lib.mkOption {
        ...
        default = null;
        description = "Fan operating mode (auto, fixed, or curve)";
      };

      level = lib.mkOption {
        type = lib.types.nullOr (lib.types.ints.between 0 5);
        default = null;
        description = "Fan speed level when mode is 'fixed' (0=0%, 1=20%, 2=40%, 3=60%, 4=80%, 5=100%)";
      };
      ...
      rampupCurve = lib.mkOption {
        ...
        description = "Temperature thresholds (°C) for fan ramp-up at levels 1-5, comma-separated";
      };
      ...
      rampdownCurve = lib.mkOption {
        ...
        description = "Temperature thresholds (°C) for fan ramp-down at levels 1-5, comma-separated";
      };
    };
  };
...
in
{
  options.hardware.strix-halo.ec-su-axb35 = {
    ...
    powerMode = lib.mkOption {
      ...
      default = null;
      description = "APU power mode (quiet, balanced, or performance)";
    };
