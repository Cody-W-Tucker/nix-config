...
pkgs.writeShellApplication {
  ...
  text = ''
    ...
    APP_KEY="''${APP_CLASS%% *}"
    APP_KEY="''${APP_KEY##*/}"
    APP_KEY_LOWER="$(printf '%s' "$APP_KEY" | tr '[:upper:]' '[:lower:]')"

    matches_app_key() {
      printf '%s\n' "$@" \
        | tr '[:upper:]' '[:lower:]' \
        | tr -cs '[:alnum:]_-' '\n' \
        | grep -Fxq "$APP_KEY_LOWER"
    }
    ...
    activate_tray_item() {
      ...
      while IFS= read -r item; do
        service="''${item%%/*}"
        path="/''${item#*/}"
        id=$(busctl --user --json=short get-property "$service" "$path" org.kde.StatusNotifierItem Id 2>/dev/null | jq -r '.data // ""' || true)
        ...
        if matches_app_key "$id" "$title" "$tooltip"; then
